...
export default function RevenueChart({ revenueData }: Props) {
  const labels = revenueData.map((v) => new Date(v._id).toLocaleDateString());
  const data = {
    labels,
    datasets: [
      {
        label: "Revenue",
        data: revenueData.map((v) => v.dailyRevenue),
        borderColor: "#3b82f6",
